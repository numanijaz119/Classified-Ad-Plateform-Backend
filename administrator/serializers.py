...
class AdminUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'phone', 'is_active', 'is_suspended', 'suspension_reason',
            'email_verified', 'created_at', 'last_login', 'total_ads', 
            'active_ads', 'pending_ads', 'featured_ads', 'days_since_joined',
            'status_display'
        ]
...
class AdminReportSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'is_reviewed', 'admin_notes', 'created_at', 'reviewed_at',
            'ad_title', 'ad_id', 'reporter_name', 'reporter_email',
            'reviewer_name', 'days_ago'
        ]
...
class AdminBannerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Banner
        fields = [
            'id', 'title', 'description', 'banner_type', 'image', 
            'html_content', 'text_content', 'position', 'target_states',
            'target_categories', 'click_url', 'open_new_tab',
            'impressions', 'clicks', 'ctr', 'start_date', 'end_date',
            'is_active', 'is_currently_active', 'priority',
            'created_by_name', 'created_at', 'updated_at',
            'target_states_display', 'target_categories_display'
        ]
        read_only_fields = ['created_by', 'created_at', 'updated_at', 'impressions', 'clicks']
    
    def get_target_states_display(self, obj):
        return [{'id': state.id, 'name': state.name} for state in obj.target_states.all()]
    ...
    def validate(self, data):
        ...
        if banner_type == 'html' and not data.get('html_content'):
            raise serializers.ValidationError("HTML content is required for HTML banners.")
        
        if banner_type == 'text' and not data.get('text_content'):
            raise serializers.ValidationError("Text content is required for text banners.")
...
class AdminStateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = State
        fields = [
            'id', 'code', 'name', 'domain', 'is_active',
            'total_ads', 'active_ads', 'users_count'
        ]
...
class AdminCategorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'is_active', 'sort_order', 'total_ads', 
            'active_ads', 'pending_ads'
        ]
...
class AdminCitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = City
        fields = [
            'id', 'name', 'state', 'state_name', 'is_active',
            'sort_order', 'total_ads'
        ]
...
class AdminSettingsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'ad_approval_required', 'featured_ad_price', 'max_images_per_ad',
            'ad_expiration_days', 'user_registration_enabled', 
            'email_verification_required', 'max_ads_per_user',
            'admin_email_notifications', 'user_email_notifications',
    ...
    def validate_max_images_per_ad(self, value):
        if value < 1 or value > 50:
            raise serializers.ValidationError("Max images per ad must be between 1 and 50.")
        return value
    
    def validate_ad_expiration_days(self, value):
        if value < 1 or value > 365:
            raise serializers.ValidationError("Ad expiration days must be between 1 and 365.")
        return value
